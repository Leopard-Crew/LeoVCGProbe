...
int main()
{
    const char *roundtripPath = "build/vcg_triangle_roundtrip_probe.stl";
    ...
    const int exportErr =
        vcg::tri::io::ExporterSTL<LeoMesh>::Save(sourceMesh, roundtripPath, false);

    int loadMask = 0;
    const int importErr =
        vcg::tri::io::ImporterSTL<LeoMesh>::Open(importedMesh, roundtripPath, loadMask);

    std::cout << "LeoVCGProbe STL roundtrip probe ran." << std::endl;
    ...
    if (importErr != 0) {
        std::cerr << "ERROR: STL import failed" << std::endl;
        return 1;
    }

    if (!fileContains(roundtripPath, "solid")
        || !fileContains(roundtripPath, "facet normal")
        || !fileContains(roundtripPath, "vertex")
        || !fileContains(roundtripPath, "endsolid")) {
        std::cerr << "ERROR: STL output did not contain expected ASCII markers" << std::endl;
        return 1;
    }
    ...
    if (importedMesh.vert.size() != 3 || importedMesh.face.size() != 1) {
        std::cerr << "ERROR: unexpected mesh size after STL roundtrip" << std::endl;
        return 1;
    }

    if (!pointEquals(importedMesh.bbox.min, 0.0f, 0.0f, 0.0f)
        || !pointEquals(importedMesh.bbox.max, 1.0f, 1.0f, 0.0f)) {
        std::cerr << "ERROR: unexpected bounding box after STL roundtrip" << std::endl;
        return 1;
    }

    std::cout << "STL roundtrip check: OK" << std::endl;
    return 0;
}
