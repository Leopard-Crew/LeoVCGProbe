...
int main()
{
    const char *roundtripPath = "build/vcg_triangle_roundtrip_probe.ply";
    ...
    if (importedMesh.vert.size() != 3 || importedMesh.face.size() != 1) {
        std::cerr << "ERROR: unexpected mesh size after PLY roundtrip" << std::endl;
        return 1;
    }

    if (!pointEquals(importedMesh.bbox.min, 0.0f, 0.0f, 0.0f)
        || !pointEquals(importedMesh.bbox.max, 1.0f, 1.0f, 0.0f)) {
        std::cerr << "ERROR: unexpected bounding box after PLY roundtrip" << std::endl;
        return 1;
    }

    std::cout << "PLY roundtrip check: OK" << std::endl;
    return 0;
}
